...
static int wait_function(void *unused)
{
        ...
        while(1) {
                ...
                if(wait_queue_flag == 2) {
                        pr_info("WaitEventCalled: User app exited\n");
                        return 0;
                }
                pr_info("WaitEventCalled: Data has been consumed for %d times\n", ++read_count);
                wait_queue_flag = 0;
        }
        do_exit(0);
        return 0;
...
static ssize_t wq_read(struct file *filp, char __user *buf, size_t len, loff_t *off)
{
	...
	char data[100]= "hi kishan, data has been consumed";
        wait_queue_flag = 1;
...
static ssize_t wq_write(struct file *filp, const char __user *buf, size_t len, loff_t *off)
{
	...
	printk(" Message from user space: %s", data);
	vfree(data);
        return len;
}
...
static int __init wq_driver_init(void)
{
        ...
        /*Creating device*/
        if((device_create(dev_class,NULL,dev,NULL,"wq_dynamic_device")) == NULL){
